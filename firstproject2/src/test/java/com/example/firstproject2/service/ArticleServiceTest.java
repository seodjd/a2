...
@SpringBootTest // 해당 클래스는 Spring Boot와 연동되어 Testing 된다.
class ArticleServiceTest {
    ...
    void index() {
        ...
        Article b = new Article(2L, "가가가가", "1111");
        Article c = new Article(3L, "가가가가", "1111");
    ...
    void show_실패____존재하지_않는_id_입력() {
        ...
        Article article = articleService.show(id);
        // 비교
        assertEquals(expected, article);    // null은 toString()으로 호출할 수 없기 때문에 지워준다.
    ...
    @Transactional
    @Test
    void create_성공_title과_content만_있는_경우() {
        // 예상
        String title = "라라라라";
        String content = "4444";
